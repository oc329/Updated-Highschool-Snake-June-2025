...
class Apple(): 
  def __init__(self, parent_screen):
    ...
    self.grid_pos = (random.randint(0, TOTAL_ROWS - 1), random.randint(0, TOTAL_COLUMNS - 1))# Minus 1 since randint upper is inclusive
  ...
  def relocate(self):
    # self.x = random.randint(0, self.window.get_width()- self.width) # -  1 self.width so it doesn't spawn off screen since x coordinate is in top left of apple
    # self.y = random.randint(0, self.window.get_height() - self.height)

    ## Getting Random Column and Row number

    #Places the apple at a random set of coors within the screen dimensions
    self.grid_pos = (random.randint(0, TOTAL_ROWS - 1), random.randint(0, TOTAL_COLUMNS - 1))
