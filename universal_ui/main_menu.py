...
from ui.text_surface import HighlightableEditableSingleLineTextSurface
from snake_game_stuff.snake import MenuSnake, GameSnake
...
class Menu: 
    ...
    def initialize_arrow(self):
        """
        Initializes the arrow highlighter next to the first default selected box.
        ** Arrow instance to be after self.selected_box since the Arrow's ininitalization 
        references self.selected_box and construction method __init__ 
        doesn't search for the attribute like calling a method does **
        """
        space = self.selected_box.get_height() // 8
        self.arrow = Arrow(self, space)
